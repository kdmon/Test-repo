    ...
    ft.addRowToggle = function() {
      ...
      for (var c in ft.columns) {
        ...
        if (col.toggle) {
          ...
          var selector = '> tbody > tr:not(.' + cls.detail + ',.' + cls.disabled + ') > td:nth-child(' + (parseInt(col.index, 10) + 1) + '),' + '> tbody > tr:not(.' + cls.detail + ',.' + cls.disabled + ') > th:nth-child(' + (parseInt(col.index, 10) + 1) + ')';
      ...
      if (!hasToggleColumn) {
        $table.find('> tbody > tr:not(.' + cls.detail + ',.' + cls.disabled + ') > td:first-child').add('> tbody > tr:not(.' + cls.detail + ',.' + cls.disabled + ') > th:first-child').not('.' + cls.detailCell).prepend($(opt.toggleHTMLElement).addClass(cls.toggle));
      }
    };
...
(function($, w, undefined) {
  ...
  function Sort() {
    ...
    p.doSort = function(columnIndex, ascending) {
      ...
      ascending = (ascending === undefined) ? $th.hasClass(cls.sorted) : (ascending === 'toggle') ? !$th.hasClass(cls.sorted) : ascending;
  ...
  function Filter() {
    ...
    p.init = function(ft) {
      ...
      if (ft.options.filter.enabled === true) {
        ...
        $(ft.table).unbind('.filtering').bind({
    ...
    p.filter = function(filterString) {
      ...
      var event = ft.raise('footable_filtering', {
        filter: filterString,
        clear: clear
      });
      if (event && event.result === false) return;
